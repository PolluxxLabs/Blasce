...
- **Detail page**: Cinematic backdrop, trailer embed, IMDB + RT + Metacritic score badges (real-time from OMDb), cast grid, episode list (TV shows by season), watchlist button, Watch Now stream player with 3 switchable sources (vidlink.pro, 2embed.cc, vidsrc.me). **IMDB enrichment**: `/api/meta` endpoint fetches full OMDB data (plot, poster, directors, writers, cast, language, country, awards, box office, runtime, PG rating) and overlays it on the Detail page — genres merged from DB+IMDB, director shown below title, rich storyline, top cast preview, detailed sidebar, awards card. TV episodes individually clickable.
