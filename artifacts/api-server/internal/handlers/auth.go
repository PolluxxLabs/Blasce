...
func UpdateProfile(w http.ResponseWriter, r *http.Request) {
        ...
        if req.DisplayName != "" {
                if _, err := db.DB.Exec(`UPDATE users SET display_name = $1 WHERE id = $2`, req.DisplayName, id); err != nil {
        ...
        if req.Password != "" {
                ...
                if _, err := db.DB.Exec(`UPDATE users SET password_hash = $1 WHERE id = $2`, string(hash), id); err != nil {
        ...
        var email, displayName, createdAt string
        if err := db.DB.QueryRow(`SELECT email, display_name, created_at::text FROM users WHERE id = $1`, id).
